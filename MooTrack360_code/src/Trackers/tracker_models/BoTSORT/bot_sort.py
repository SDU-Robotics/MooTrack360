    ...
    def update(self, output_results, img):
        ...
        if len(output_results):
            if output_results.shape[1] == 5:
                ...
                # If no class information is provided, you could default to a constant
                classes = np.zeros_like(scores, dtype=int)
            elif output_results.shape[1] == 6:
                scores = output_results[:, 4]         # Use column 4 as the score
                bboxes = output_results[:, :4]         # x1, y1, x2, y2
                classes = output_results[:, 5]  # Use column 5 as the class id
            else:
        ...
        if len(dets) > 0:
            '''Detections'''
            if self.args.with_reid:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, cls, f) for
                              (tlbr, s, cls, f) in zip(dets, scores_keep,classes_keep, features_keep)]
            else:
        ...
        # association the untrack to the low score detections ## added cls
