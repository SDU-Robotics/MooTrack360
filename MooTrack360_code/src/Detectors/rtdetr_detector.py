...
class RTDETRDetector (BaseDetector):
    """
    A concrete detector that uses the Ultralytics RTDETR model (YOLOv8).
    """

    def __init__(self, model_path: str, device: str = "cuda", predict_params_path: str = "detector_configs.yaml"):
        """
        :param model_path: Path to the Ultralytics RTDETR model (.pt file), e.g. 'RTDETR.pt'
        ...
        try:
            ...
            self.predict_params = config["RTDETR_predict"]
    ...
    def detect_objects(self, frame: np.ndarray) -> List[Detection]:
        ...
        results = self.model.predict(frame, **self.predict_params)  # conf=0.25 as an example
